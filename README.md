...
Скрипт `deploy_script.sh` позволяет быстро выполнить вышеуказанные операции одним запуском команды

```
`./deploy_script.sh`
```

Полезно при разработе и отладке кода - последние обновления автоматически скачиваются с GitHub, обновляются пакеты, пересобирется фронтенд, применяются миграции. При этом Nginx-сервер продолжает работать и не перезапускается.
